...
- 题型：空间、动态与路径
- 母问题：繁殖周期、羊群状态与羊栏占用耦合的生产计划
- 问题链：繁殖阶段时间轴 -> 各类羊栏占用 -> 最小栏数/最大产出 -> 112栏具体排程 -> 不确定性缓冲
